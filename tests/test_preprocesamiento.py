...
def test_equivale_a_transformar_a_mano():
    ...
    m1, m2 = fuse(con_param, **COMUN), fuse(a_mano, **COMUN)
    for tipo in m1.G:
...
def test_transform_aplica_el_preprocesamiento_del_fit():
    ...
    m1 = fuse(_con(base, "r01", preprocess="log1p"), **COMUN)
    m2 = fuse(_con(base, "r01", matrix=_log1p(base["r01"].matrix)), **COMUN)
...
def test_idf_en_transform_usa_el_estado_del_entrenamiento():
    ...
    m1 = fuse(_con(base, "r01", preprocess="idf"), **COMUN)
    m2 = fuse(_con(base, "r01", matrix=M.multiply(idf_entrenamiento[None, :]).tocsr()),
             **COMUN)
    d1 = m1.transform({"r01": Relation(src="t1", dst="t2", matrix=nueva)},
                      target="t1")
...
def test_declaracion_inconsistente_se_rechaza():
    base = _base_no_negativa()
    nueva = base["r01"].matrix[:10]
    sin_prep = fuse(base, **COMUN)
    with pytest.raises(ValueError, match="declares"):
        sin_prep.transform({"r01": Relation(src="t1", dst="t2", matrix=nueva,
                                            preprocess="log1p")}, target="t1")
    con_prep = fuse(_con(base, "r01", preprocess="log1p"), **COMUN)
    with pytest.raises(ValueError, match="declares"):
...
def test_idf_no_aplica_a_columnas_nuevas():
    base = _base_no_negativa()
    modelo = fuse(_con(base, "r01", preprocess="idf"), **COMUN)
    nueva = base["r01"].matrix[:, :10]
...
def test_save_load_conserva_el_estado(tmp_path):
    base = _base_no_negativa()
    nueva = base["r01"].matrix[:10]
    modelo = fuse(_con(base, "r01", preprocess=("log1p", "idf")), **COMUN)
...
def test_no_muta_los_datos_del_usuario():
    ...
    fuse(_con(base, "r01", matrix=matriz, preprocess="log1p"), **COMUN)
    assert np.array_equal(matriz.data, datos_antes)


def test_reconstruct_entries_invierte_la_cadena():
    base = _base_no_negativa()
    modelo = fuse(_con(base, "r01", preprocess="log1p"), **COMUN)
...
def test_resume_reaprende_el_preprocesamiento():
    base = _base_no_negativa()
    con_param = _con(base, "r01", preprocess="log1p")
    modelo = fuse(con_param, ranks=RANKS, max_iter=10, tol=None, random_state=0)
...
def test_poisson_con_preprocesamiento_idf():
    ...
    modelo = fuse(R, {"fila": 3, "columna": 3}, max_iter=30, tol=None,
                 random_state=0)
...
def test_validaciones():
    ...
    with pytest.raises(ValueError, match="non-negative"):
        fuse({"r": Relation(src="a", dst="b", matrix=negativa, preprocess="sqrt")},
            {"a": 2, "b": 2}, max_iter=2)
