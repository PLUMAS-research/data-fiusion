...
from datafiusion import FusionModel, fit
...
def test_save_load_con_supervision(tmp_path):
    ...
    modelo = fit(base, RANKS, supervision={"t1": permitido}, max_iter=15,
...
def test_save_load_con_masks_y_graphs(tmp_path):
    ...
    modelo = fit(base, RANKS, masks={"r01": mascara}, graphs=grafo,
...
def test_save_sobre_directorio_reutilizado_limpia_lo_anterior(tmp_path):
    ...
    base = instancia()
    con_extras = fit(base, RANKS, masks={"r01": np.arange(0, 80, 2)},
                     max_iter=10, tol=None, random_state=0)
    con_extras.save(tmp_path / "modelo")
    sin_extras = fit(base, RANKS, max_iter=10, tol=None, random_state=0)
...
def test_save_no_conserva_archivos_huerfanos_del_save_anterior(tmp_path):
    base = instancia()
    dos = fit(base, RANKS, masks={"r01": np.arange(0, 80, 2),
                                  "r12": np.arange(0, 60, 3)},
              max_iter=10, tol=None, random_state=0)
    dos.save(tmp_path / "modelo")
    una = fit(base, RANKS, masks={"r01": np.arange(0, 80, 2)},
              max_iter=10, tol=None, random_state=0)
...
def test_escalares_numpy_en_params_sobreviven_el_save(tmp_path):
    ...
    modelo = fit(instancia(), RANKS, weights={"r01": np.float32(30.0)},
